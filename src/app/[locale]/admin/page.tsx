  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center">
        ...
        <h1 className="text-3xl font-bold mb-2 text-zinc-900 dark:text-zinc-50">Unauthorized Access</h1>
        <p className="text-zinc-500 dark:text-zinc-400 max-w-md mb-8">
          Your wallet <span className="font-mono text-zinc-900 dark:text-zinc-100">{publicKey?.slice(0, 8)}...</span> is not registered as the administrator.
        </p>
        <Link href="/" className="px-8 py-3 bg-white border border-zinc-200 rounded-2xl font-bold hover:bg-zinc-50 transition shadow-sm">
          Return Home
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-4">
             <div className="bg-zinc-50 dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700 rounded-2xl px-5 py-3 shadow-sm">
                <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest block mb-1">{t('contractAdmin')}</span>
                <span className="font-mono text-sm font-bold text-zinc-900 dark:text-zinc-100">{adminAddress}</span>
             </div>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12 motion-safe:animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <StatsCard icon={<ShieldAlert className="text-amber-500" />} label={t('platformFee')} value={platformFee !== null ? basisPointsToPercentage(platformFee) : '...'} />
      </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
             {pendingCampaigns.length === 0 ? (
               <div className="p-20 text-center flex flex-col items-center">
                  ...
                  <p className="text-lg font-bold text-zinc-900 dark:text-zinc-50 mb-1">{t('noPending')}</p>
                  <p className="text-sm text-zinc-500">{t('subtitle')}</p>
               </div>
             ) : (
               <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                  {pendingCampaigns.map(c => (
                    <article key={c.id} className="p-8 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors group">
                       <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                          <div className="flex-1">
                             <div className="flex items-center gap-3 mb-3">
                                <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200/50 dark:border-amber-800">#{c.id}</span>
                                <span className="text-xs font-bold text-zinc-400 capitalize">{CATEGORY_LABELS[c.category as Category]}</span>
                             </div>
                             <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 mb-2 group-hover:text-amber-600 transition-colors">{c.title}</h3>
                             <p className="text-sm text-zinc-500 dark:text-zinc-400 line-clamp-2 max-w-2xl mb-4">{c.description}</p>
                             <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs font-bold">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="font-mono text-zinc-600 dark:text-zinc-300">{c.creator.slice(0, 6)}...{c.creator.slice(-6)}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <span className="text-zinc-400">{t('goal')}:</span>
                                  <span className="text-zinc-900 dark:text-zinc-100">{stroopsToXlm(BigInt(c.funding_goal)).toLocaleString()} XLM</span>
                                </div>
                             </div>
                          </div>
                          
                          <div className="flex md:flex-col items-center gap-3">
                             <Link href={`/causes/${c.id}`} className="flex-1 md:w-full inline-flex items-center justify-center gap-2 px-6 py-3 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm font-bold hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                                {t('reviewDetails')}
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
           ...
           <section className="bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 p-8 shadow-sm">
              ...
              <form onSubmit={handleUpdateFee} className="space-y-6">
                 <div>
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 mb-3">{t('feeInBasisPoints')}</label>
                    <input 
                      type="number" 
                      value={feeInput} 
           ...
           <section className="bg-gradient-to-br from-red-50 to-rose-100/50 dark:from-red-900/10 dark:to-zinc-900 rounded-[2.5rem] border border-red-100 dark:border-red-900/20 p-8 shadow-sm">
              ...
              <form onSubmit={handleTransferAdmin} className="space-y-6">
                 <div>
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-red-400 mb-3">{t('newAdminAddress')}</label>
                    <input 
                      type="text" 
                      placeholder="G..."
                      value={newAdminInput}
...
function StatsCard({ icon, label, value }: { icon: React.ReactNode, label: string, value: string | number }) {
  return (
    <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-zinc-100 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow group">
       ...
       <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 mb-2 block">{label}</span>
